...
class AccountMove(models.Model):
    ...
    def _get_invoice_payment_widget(self):
        if isinstance(self.invoice_payments_widget, str):
            j = json.loads(self.invoice_payments_widget)
        elif isinstance(self.invoice_payments_widget, dict):
            j = self.invoice_payments_widget
        else:
            j = {}
        return j.get('content', []) if j else []
    ...
    @api.depends('line_ids', 'line_ids.price_subtotal', 'line_ids.tax_line_id', 'state', 'payment_state','line_ids.balance')
        ...
        for inv in self:
            ...
            if inv.state != 'draft' and tax_line_ids:
                for line in tax_line_ids:
                    ...
                    # A VERIFICAR ESTE CODIGO DE ABAJO |||||||||||||||||||
    ...
    def norma_recompute(self):
        ...
        for field_name, field_info in self.fields_get().items():
            # Verificar si el campo es almacenado y tiene dependencias
            if field_info.get("store") and field_info.get("depends"):
                field = self._fields.get(field_name)
                if field and field.compute:  # Asegurarse de que el campo sea computado
                    self.env.add_to_compute(field, invoice_ids)
                else:
